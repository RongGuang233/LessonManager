...
export function statsFor(state,start,end,studentId='') {
  ...
  return {minutes:courses.reduce((sum,c)=>sum+(c.actual_minutes??0),0),fee:courses.reduce((sum,c)=>sum+(c.fee_cents??0),0),received:payments.filter(p=>p.kind!=='adjustment').reduce((sum,p)=>sum+p.amount_cents,0),adjusted:payments.filter(p=>p.kind==='adjustment').reduce((sum,p)=>sum+p.amount_cents,0),unknown:courses.filter(c=>c.actual_minutes==null||c.fee_cents==null||c.needs_review).length,courses,payments};
}
