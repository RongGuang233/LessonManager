...
let studentAttention='',studentTab=studentLocation.tab;
const recordRanges={ledger:{mode:'recent',start:addDays(today(),-30),end:today()},courses:{mode:'upcoming',start:today(),end:today()}};
let {mode:recordMode,start:recordStart,end:recordEnd}=recordRanges[studentTab]||recordRanges.ledger;
...
let dialogDraftKey='';
const dialogParents=[];
let recordKind='',courseStatus='scheduled',statsOrigin='';
...
function newCourseSummary(course){
 if(!course||!studentBy(course.student_id)||!course.subject||!Number.isInteger(course.duration_minutes)||course.duration_minutes<60||course.duration_minutes>1440||course.duration_minutes%60)return '';
 const fee=estimatedFee(course,studentBy(course.student_id)),end=minutes(course.start_time)+course.duration_minutes;
 const ending=(end>=1440?'次日 ':'')+timeLabel(end%1440);
 const price=fee==null?'单价待设置':fee===0?'免费':`预计 ${money(fee)}`;
 const weekday='日一二三四五六'[new Date(course.date+'T12:00:00').getDay()];
 return `<div class="change-summary new-course-summary"><strong>${dateShort(course.date)} 周${weekday} ${course.start_time}–${ending} · ${esc(course.subject)} · ${price}</strong></div>`;
}
function courseForm(c=null,defaults={}){
 if(!state.students.length){studentForm();return;}
 const item={student_id:contextualStudentId(route,{selectedStudent,filterStudent,statsStudent},state.students),date:anchor,start_time:'18:00',duration_minutes:120,notes:'',...c,...defaults};
 ...
 const preview=()=>{const f=Object.fromEntries(new FormData(dialog.querySelector('form'))),changes=plannedChanges(state.courses,c,f),conflicts=scheduleConflicts(state.courses,changes);dialog.querySelector('#schedule-preview').innerHTML=`${c?(f.scope==='following'?changePreview(c,changes):singleChangePreview(c,changes[0])):newCourseSummary(changes[0])}${changes.length>1&&!(c&&f.scope==='following')?notice(`将${c?'修改':'安排'} ${changes.length} 节课：${changes.slice(0,4).map(x=>dateShort(x.date)).join('、')}${changes.length>4?'等':''}。已完成课程保持原记录。`):''}${conflicts.length?notice(`<strong>有 ${conflicts.length} 处时间重叠</strong><ul>${conflicts.slice(0,5).map(({change,other})=>`<li>${dateShort(change.date)} ${change.start_time} 与 ${esc(studentBy(other.student_id)?.name)} ${other.start_time}–${timeLabel(minutes(other.start_time)+displayMinutes(other))} 重叠</li>`).join('')}</ul><label class="check"><input type="checkbox" name="overlap_ack">已知晓重叠，仍按此安排</label>`,'warning'):''}`;};
 dialog.querySelectorAll('[name="date"],[name="start_time"],[name="duration_hours"],[name="repeat_until"],[name="scope"],[name="notes"],[name="subject"]').forEach(x=>x.addEventListener('input',preview));
...
function selectStudentTab(tab){
 if(recordRanges[studentTab])recordRanges[studentTab]={mode:recordMode,start:recordStart,end:recordEnd};
 studentTab=tab;
 if(recordRanges[tab])({mode:recordMode,start:recordStart,end:recordEnd}=recordRanges[tab]);
}
function selectStudent(id,tab='overview'){
 selectStudentTab('overview');
 selectedStudent=id;statsOrigin='';recordKind='';courseStatus='scheduled';recordLimit=30;
 recordRanges.courses={mode:'upcoming',start:today(),end:today()};
 selectStudentTab(tab);
}
function rangeCaption(mode,start,end){
 if(mode==='all')return '';
 const text=mode==='upcoming'?`${today()} 起`:start&&end?`${start} — ${end}`:'日期待定';
 return `<span class="range-caption">${esc(text)}</span>`;
}
function rangeControls(){return `<div class="record-controls"><label>记录范围 <select id="record-mode">${studentTab==='courses'?option('upcoming','接下来',recordMode):''}${option('recent','近 30 天',recordMode)}${option('all','全部记录',recordMode)}${state.periods.map(p=>option('period:'+p.id,periodLabel(p),recordMode)).join('')}${option('custom','自定义',recordMode)}</select></label>${recordMode==='custom'?`<label>从 <input id="record-start" type="date" value="${recordStart}"></label><label>至 <input id="record-end" type="date" value="${recordEnd}"></label>`:rangeCaption(recordMode,recordStart,recordEnd)}</div>`;}
...
function studentDetail(s){
 ...
 if(studentTab==='profile')content=studentProfile(s,pending);
  ...
  const includeUndated=!pendingPeriod&&recordMode!=='upcoming'&&(recordMode==='all'||statsOrigin!==s.id);
  ...
  if(recordMode==='upcoming')filtered.reverse();
  const pendingAction=studentTab==='courses'&&!filtered.length&&['','scheduled'].includes(courseStatus)&&courses.some(c=>c.status==='scheduled')?button('all-pending-courses','查看全部待上课','','text-button'):'';
  content=`<div class="record-toolbar">${rangeControls()}${recordTypeControls()}</div>`+(recordStart>recordEnd&&recordMode!=='all'?notice('开始日期不能晚于结束日期。','warning'):'')+(studentTab==='ledger'?`<details class="record-help"><summary>记录说明</summary><p>有日期的记录按时间倒序显示；未填日期的记录单列。历史余额已核对的明细不重复影响当前余额，逐笔结余无法确定时不作推算。</p></details>${dated.length||undated.length?accountTable(dated.slice(0,recordLimit),undated,s.balance_verified):'<p class="quiet-empty">所选范围暂无流水</p>'}${dated.length>recordLimit?button('more-records',`再显示 30 条（共 ${dated.length} 条）`):''}`:courseTable(filtered.slice(0,recordLimit),pendingAction)+(filtered.length>recordLimit?button('more-records',`再显示 30 条（共 ${filtered.length} 条）`):''));
...
function renderStudents(){
 const students=state.students.filter(s=>(!studentStatus||s.status===studentStatus)&&studentMatchesSearch(s,studentSearch)&&(!studentAttention||attentionFor(s,state.courses)===studentAttention));
 if(!students.some(s=>s.id===selectedStudent)){selectStudent(students[0]?.id||'');}
 saveStudentLocation(plannerStorage,selectedStudent,studentTab);
 const archivedMatches=!students.length?archivedSearchMatches(state.students,studentSearch,studentStatus):[];
 const noSelection=!state.students.length?empty('添加学生后即可安排课程、记缴费。'):archivedMatches.length?empty(`已归档中找到 ${archivedMatches.length} 位匹配学生。`,`<div class="actions search-empty-actions">${button('show-archived-matches','查看已归档学生','','primary')}${button('reset-student-filters','清除筛选','','text-button')}</div>`):empty('请调整搜索条件或学生状态。',button('reset-student-filters','清除筛选','','text-button'));
 main.innerHTML=header('','学生账页',!students.length&&state.students.length?'没有匹配的学生':`${students.length} 位学生`,button('new-student','添加学生'))+`<div class="attention-filters"><button data-action="attention" data-id="" aria-pressed="${!studentAttention}">全部</button>${['debt','low','unverified'].map(k=>({key:k,count:state.students.filter(s=>(!studentStatus||s.status===studentStatus)&&studentMatchesSearch(s,studentSearch)&&attentionFor(s,state.courses)===k).length})).filter(x=>x.count||x.key===studentAttention).map(x=>`<button data-action="attention" data-id="${x.key}" aria-pressed="${studentAttention===x.key}">${attentionLabels[x.key]} <b>${x.count}</b></button>`).join('')}</div><div class="students-layout"><section class="student-index"><div class="student-controls"><label class="sr-only" for="student-search">搜索姓名或年级</label><input id="student-search" type="search" placeholder="搜索姓名或年级" value="${esc(studentSearch)}"><label class="sr-only" for="student-status">学生状态</label><select id="student-status">${option('','全部状态',studentStatus)}${['active','paused','archived'].map(s=>option(s,statusText(s),studentStatus)).join('')}</select><label class="sr-only" for="student-select">选择学生</label><select id="student-select">${students.map(s=>option(s.id,`${s.name} · ${s.balance_verified&&s.balance_cents<0?'欠费 '+money(-s.balance_cents):money(s.balance_cents)}${s.balance_verified?'':' · 待核对'}`,selectedStudent)).join('')}</select></div><div class="student-list">${students.map(s=>`<button class="student-row ${s.id===selectedStudent?'selected':''}" data-action="select-student" data-id="${esc(s.id)}"><span><strong>${esc(s.name)}</strong>${s.grade||s.status!=='active'?`<small>${[s.grade,s.status!=='active'?statusText(s.status):''].filter(Boolean).map(esc).join(' · ')}</small>`:''}</span><span class="student-balance">${balanceHTML(s)}${attentionFor(s,state.courses)==='low'?'<small class="negative">不足下次课费</small>':''}</span></button>`).join('')}</div></section><section class="student-detail">${selectedStudent?studentDetail(studentBy(selectedStudent)):noSelection}</section></div>`;
 ...
 document.querySelector('#student-select').onchange=e=>{selectStudent(e.target.value);render();window.scrollTo(0,0);};
 const type=document.querySelector('#record-type');if(type)type.onchange=e=>{if(studentTab==='ledger')recordKind=e.target.value;else{courseStatus=e.target.value;if(recordMode==='upcoming'&&['completed','cancelled'].includes(courseStatus)){recordMode='recent';recordStart=addDays(today(),-30);recordEnd=today();}}recordLimit=30;render();};
 const mode=document.querySelector('#record-mode');if(mode)mode.onchange=e=>{recordMode=e.target.value;recordLimit=30;if(recordMode==='upcoming'){courseStatus='scheduled';recordStart=today();recordEnd=today();}else if(recordMode==='recent'){recordStart=addDays(today(),-30);recordEnd=today();}else if(recordMode.startsWith('period:')){const p=state.periods.find(p=>p.id===recordMode.slice(7));recordStart=p.start;recordEnd=p.end;}else if(recordMode==='custom'&&(!recordStart||!recordEnd)){recordStart=addDays(today(),-30);recordEnd=today();}render();};
...
function statementForm(id){
 ...
 const {start:initialStart,end:initialEnd}=statementRange({tab:route==='students'&&selectedStudent===id?studentTab:'overview',mode:recordMode,start:recordStart,end:recordEnd,pending:state.periods.some(p=>recordMode==='period:'+p.id&&p.range_kind==='pending')});
 const latestPayment=state.payments.filter(p=>p.student_id===id&&p.kind==='payment'&&p.date&&p.date<=today()).sort((a,b)=>b.date.localeCompare(a.date))[0];
...
function openStudent(id,tab='overview'){selectStudent(id,tab);studentSearch='';studentAttention='';studentStatus=studentBy(id)?.status||'active';if(dialog.open)closeDialog();if(route==='students')render();else location.hash='students';}
async function handleAction(action,id,el){
 ...
 case'select-student':selectStudent(id);render();window.scrollTo(0,0);break;
 ...
 case'all-student-records':selectStudentTab('ledger');recordMode='all';recordKind='';recordLimit=30;statsOrigin='';render();break;
 case'student-tab':selectStudentTab(id);recordLimit=30;render();break;case'more-records':recordLimit+=30;render();break;
 case'all-pending-courses':recordMode='all';courseStatus='scheduled';recordLimit=30;render();break;
 case'show-archived-matches':studentStatus='archived';studentAttention='';selectedStudent='';render();break;
 case'reset-student-filters':studentSearch='';studentStatus='active';studentAttention='';render();break;
...
function renderStats(){
 ...
 main.innerHTML=statsHeader()+`${statsStart>statsEnd?notice('开始日期不能晚于结束日期。','warning'):''}${period?.range_kind==='coverage'?notice(`学期日期尚未确定，当前按已录入范围统计。 ${button('edit-period','设置学期日期',period.id,'text-button')}`):''}<div class="stat-band"><div><span>实际授课</span><strong>${result.courses.length}<small> 节课 · ${Number((result.minutes/60).toFixed(2))} 小时</small></strong></div><div><span>已上课费</span><strong>${money(result.fee)}</strong></div><div><span>${corrections?'核对后净收款':'净收款'}</span><strong>${money(result.received)}</strong></div></div>${result.undatedCourses||result.undatedPayments?notice(`${result.undatedCourses} 节已上课、${result.undatedPayments} 笔款项日期未知，${statsMode==='all'?'已纳入全部合计':'未纳入此时段'}。 ${button('all-reviews','查看历史核对')}`):''}${result.unknown?notice(`${result.unknown} 节已完成课程待核对；未知课时与费用未计入相应合计。`):''}<details class="stats-explanation compact-details"><summary>统计口径${corrections?' · 含历史收款核对':''}</summary><p>授课节数按已完成课程逐节计数，1 小时和 2 小时课程均算 1 节。实际课时和已上课费只统计已完成课程。净收款为缴费减退款，余额调整单独列示。</p>${corrections?`<p>当前净收款包含历史收款核对 ${money(result.receiptCorrection)}；它是旧收款记录的修正，具体依据见学生流水。</p>`:''}${result.adjusted?`<p>本区间余额调整 ${money(result.adjusted)}，未计入净收款。</p>`:''}</details><div class="section-line"><h3>按学生明细</h3><span class="muted">${statsMode==='all'?'全部时间':`${statsStart} — ${statsEnd}`}</span></div>${rows.length?`<div class="table-scroll"><table><thead><tr><th>学生</th><th class="number">实际授课</th><th class="number">课费</th><th class="number">${corrections?'核对后净收款':'净收款'}</th>${adjustments?'<th class="number">余额调整</th>':''}${reviews?'<th>核对</th>':''}</tr></thead><tbody>${rows.map(r=>`<tr><td>${button('stats-ledger',esc(r.s.name),r.s.id,'text-button')}</td><td class="number">${button('stats-courses',`${r.courses.length} 节课 · ${hours(r.minutes)}`,r.s.id,'text-button')}</td><td class="number">${button('stats-courses',money(r.fee),r.s.id,'text-button')}</td><td class="number">${button('stats-receipts',money(r.received),r.s.id,'text-button')}</td>${adjustments?`<td class="number">${button('stats-adjustments',money(r.adjusted),r.s.id,'text-button')}</td>`:''}${reviews?`<td>${r.unknown||r.undatedCourses||r.undatedPayments?`${r.unknown} 节待核对 · ${r.undatedCourses+r.undatedPayments} 条缺日期`:'—'}</td>`:''}</tr>`).join('')}</tbody></table></div>`:empty('这个区间还没有已完成课程或缴费记录。')}`;
 bindStats();
}
