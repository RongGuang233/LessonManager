...
import {estimatedFee,attentionFor,plannedChanges,scheduleConflicts,reviewCategory,lessonCredit,impactsCurrentAccount,courseHasEnded,backupSummary,courseChangePayload,contextualStudentId,readPlannerView,savePlannerView} from './workflows.js';
...
dialog.addEventListener('cancel',saveDialogDraft);
function openDialog(title,body,onSubmit,footer='保存',wide=false,draftKey=''){
 if(dialog.open){saveDialogDraft();dialog.close();}dialogDraftKey=draftKey;dialog.classList.toggle('wide',wide);dialog.classList.toggle('statement-dialog',title==='家长对账');
 ...
 dialog.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>{saveDialogDraft();dialog.close();});dialog.showModal();
 form.addEventListener('input',saveDialogDraft);form.addEventListener('change',saveDialogDraft);
 form.onsubmit=async e=>{e.preventDefault();const submit=e.submitter,error=form.querySelector('.form-error');error.hidden=true;if(submit)submit.disabled=true;try{const result=await onSubmit(Object.fromEntries(new FormData(form)));if(result!==false){formDrafts.delete(draftKey);dialogDraftKey='';dialog.close();}}catch(err){error.textContent=err.message;error.hidden=false;}finally{if(submit)submit.disabled=false;}};
...
function periodLabel(p){return p.name.replace(/课表$/,'')+(p.range_kind==='coverage'?' · 已录入范围':'');}
...
function accountTable(entries,undated=[],verified=true,compact=false){
 ...
 return `<div class="table-scroll"><table class="account-table"><thead><tr><th>日期 / 事项</th><th class="number">金额</th>${compact?'':`<th class="number">${verified?'结余':'已记录结余'}</th>`}<th></th></tr></thead><tbody>${all.map(e=>{
  ...
  return `<tr><td><span class="record-title">${title}</span><small>${esc(e.date||'日期待补')}</small></td><td class="number ${e.amount<0?'negative':''}">${e.type==='settlement'?'—':e.amount==null?'待核对':`${e.amount>0?'+':''}${money(e.amount)}`}</td>${compact?'':`<td class="number ${e.balance<0?'negative':''}">${e.settled?'—':money(e.balance)}</td>`}<td>${button('entry-detail','详情',e.id,'text-button')}</td></tr>`;
...
function entryDetail(id){
 ...
 openDialog(label,`<p class="detail-lead">${money(e.type==='settlement'?e.record.balance_cents:e.amount)}</p><dl class="key-values"><div><dt>日期</dt><dd>${esc(e.date||'待补充')}</dd></div>${e.type==='settlement'?'<div><dt>类型</dt><dd>历史结余，不计入收款</dd></div>':''}</dl>${e.settled?'<p class="muted">已纳入历史余额核对。</p>':''}${e.notes?`<p class="preserve">${esc(e.notes)}</p>`:''}${e.source?`<details><summary>原始来源</summary><p class="source">${esc(e.source)}</p></details>`:''}${e.type==='settlement'?'':`<div class="dialog-actions">${button('edit-payment','更正记录',e.id)}</div>`}`,null);
}
...
function renderStudents(){
 ...
 document.querySelector('#student-select').onchange=e=>{selectedStudent=e.target.value;studentTab='overview';recordLimit=30;render();window.scrollTo(0,0);};
...
function studentForm(s=null){
 ...
 openDialog(s?'编辑学生档案':'添加学生',`<div class="form-grid">${field('姓名',input('name','text',s?.name||'','required maxlength="80"'))}${field('年级',input('grade','text',s?.grade||'','maxlength="80"'))}${field('状态',`<select name="status">${['active','paused','archived'].map(v=>option(v,statusText(v),s?.status||'active')).join('')}</select>`)}</div><fieldset class="subject-fields"><legend>学习科目与小时价</legend><p class="muted small">勾选实际学习的科目，再填写价格。</p>${subjects.map((subject,i)=>`<div class="subject-rate"><label class="check"><input name="subject_${i}" type="checkbox" value="${esc(subject)}" ${chosen.includes(subject)?'checked':''}>${esc(subject)}</label>${field(`${esc(subject)}小时价（元）`,input('rate_'+i,'number',s?.rates?.[subject]==null?'':s.rates[subject]/100,`min="0" step="0.01" ${chosen.includes(subject)?'required':'disabled'}`))}</div>`).join('')}</fieldset>${field('备注',`<textarea name="notes" rows="2">${esc(s?.notes||'')}</textarea>`)}<p class="muted small">调价用于今后确认的课程，已完成课费保留当时价格。</p>`,async f=>{
  ...
  const result=await api(s?`/students/${s.id}`:'/students',s?'PATCH':'POST',{name:f.name.trim(),grade:f.grade,status:f.status,notes:f.notes,rates});selectedStudent=result.id;studentSearch='';studentAttention='';studentStatus=result.status;await refresh();toast('学生档案已保存');
...
function paymentForm(studentId=null,p=null){
 ...
 openDialog(p?.kind==='receipt_correction'?'更正历史收款核对':p?'更正缴费记录':'记缴费',`${studentId?`<p class="detail-lead">${esc(studentBy(studentId)?.name)}</p>`:''}<div class="form-grid">${studentId?'':studentPicker(selected)}${field('类型',`<select name="kind">${p?.kind==='receipt_correction'?option('receipt_correction','历史收款核对',p.kind):option('payment','缴费',p?.kind||'payment')+option('refund','退款',p?.kind||'payment')}${p?.kind==='adjustment'?option('adjustment','余额调整',p.kind):''}</select>`)}${field('日期',input('date','date',p?(p.date||''):today(),p&&!p.date?'disabled':'required'))}${allowUnknown?field('历史日期','<span class="check"><input name="date_unknown" type="checkbox" '+(!p.date?'checked':'')+'>日期尚未核实</span>','full'):''}${field('金额（元）',input('amount','number',p?(p.kind==='receipt_correction'?p.amount_cents:Math.abs(p.amount_cents))/100:'',`required ${p?.kind==='receipt_correction'?'':`min="${p?.kind==='adjustment'?'0':'0.01'}"`} step="0.01"`))}${noteField(p?.notes||'')}</div><div id="payment-preview" class="completion-preview" aria-live="polite"></div>${p&&!p.date?notice('这是一笔日期未知的历史款项。只改金额或备注会保留未知日期，不计入某月收款。'):''}${matching.length&&unique?notice(`补齐日期后，将同时完成对应的 ${matching.length} 条缴费日期核对。`):''}${p?.kind==='adjustment'?notice('余额调整保持原有正负方向，不计入实际收款。'):''}${p?.kind==='receipt_correction'?notice('正数补足历史收款，负数扣除多计部分；核对依据见备注。'):''}${p?.source?`<details><summary>原始来源</summary><p class="source">${esc(p.source)}</p></details>`:''}${settled?notice('此款项已纳入历史余额核对，更正金额或日期只补充旧明细，当前余额保持不变。'):''}${p&&!settled?`<div class="dialog-actions">${button('delete-payment','删除此笔记录',p.id,'text-button danger')}</div>`:''}`,async f=>{
  ...
  await mutate(p?`/payments/${p.id}`:'/payments',p?'PATCH':'POST',payload,settled?'历史款项已保存，当前余额保持不变':'款项已保存，余额已更新');
...
function renderSettings(){
 ...
 main.innerHTML=header('','设置','学期与本地备份')+`<section class="settings-section"><div class="section-title"><h2>数据与备份</h2></div><div class="settings-content"><dl class="key-values"><div><dt>最近备份</dt><dd>${esc(readableTime(state.meta.last_backup_at))}</dd></div></dl><div class="actions">${button('backup','下载备份','','primary')}${button('restore','从备份恢复')}</div><details class="data-location compact-details"><summary>本地数据位置</summary><p class="path">${esc(state.meta.data_dir)}</p></details></div></section><section class="settings-section"><div class="section-title"><h2>当前学期</h2></div><div class="settings-content">${current?row(current):'<p class="muted">暂无当前学期。</p>'}${button('new-period','添加学期')}${former.length?`<details class="past-periods"><summary>其他学期与历史范围 · ${former.length} 个</summary>${former.map(row).join('')}</details>`:''}</div></section><p class="version">课时簿 ${esc(state.meta.app_version)} · 本地使用</p>`;
}
...
function openStudent(id,tab='overview'){selectedStudent=id;studentSearch='';studentAttention='';studentStatus=studentBy(id)?.status||'active';studentTab=tab;recordLimit=30;if(dialog.open)dialog.close();if(route==='students')render();else location.hash='students';}
async function handleAction(action,id,el){
 ...
 case'student-reviews':reviewScope='all';reviewStudent=id;reviewType='';reviewStatus='pending';reviewPage=0;if(dialog.open)dialog.close();if(route==='reviews')render();else location.hash='reviews';break;
...
function periodForm(p=null){
 const coverage=p?.range_kind==='coverage';
 openDialog(p?'编辑学期':'添加学期',`<div class="form-grid">${field('名称',input('name','text',p?.name.replace(/课表$/,'')||'','required placeholder="例如：秋季学期"'),'full')}${field('日期依据',`<select name="range_kind">${option('term','已确定实际学期日期',coverage?'coverage':'term')}${p?.source_start?option('coverage','学期日期待定，保留已录入范围',coverage?'coverage':'term'):''}</select>`,'full')}${field('实际开始日期',input('start','date',coverage?'':p?.start||today(),coverage?'disabled':'required'))}${field('实际结束日期',input('end','date',coverage?'':p?.end||'',coverage?'disabled':'required'))}</div>${p?.source_start?`<p class="muted small">原表已录入范围：${p.source_start} — ${p.source_end}。仅表示资料覆盖日期。</p>`:''}`,async f=>{
  const useCoverage=f.range_kind==='coverage';const start=useCoverage?p.source_start:f.start,end=useCoverage?p.source_end:f.end;
  if(!start||!end||start>end)throw new Error('请填写有效的学期起止日期。');
  ...
  if(p&&statsMode==='period:'+p.id){setStatsRange(statsMode);render();}
 },'保存',false,`period:${p?.id||'new'}`);
 dialog.querySelector('[name="range_kind"]').onchange=e=>{const pending=e.target.value==='coverage';for(const name of ['start','end']){const el=dialog.querySelector(`[name="${name}"]`);el.disabled=pending;el.required=!pending;}};
}
function restoreForm(){
 let backup=null;
 openDialog('从备份恢复',`${field('选择课时簿 JSON 备份','<input type="file" name="backup_file" accept="application/json,.json" required>')}<div id="restore-preview" aria-live="polite"></div><div id="restore-confirm" hidden>${notice('恢复会替换当前全部记录；操作前自动保留一份当前账本备份。')}${field('确认覆盖',input('confirmation','text','','required placeholder="请输入：覆盖恢复" autocomplete="off"'))}</div>`,async f=>{
