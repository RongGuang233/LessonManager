...
function openDialog(title,body,onSubmit,footer='保存',wide=false,draftKey=''){
 ...
 form.onsubmit=async e=>{e.preventDefault();const submit=e.submitter,error=form.querySelector('.form-error');error.hidden=true;if(submit)submit.disabled=true;try{const result=await onSubmit(Object.fromEntries(new FormData(form)));if(result!==false){formDrafts.delete(draftKey);dialogDraftKey='';dialog.close();if(typeof result==='function')result();}}catch(err){error.textContent=err.message;error.hidden=false;}finally{if(submit)submit.disabled=false;}};
...
function header(_eyebrow,title,subtitle,actions=''){return `<header class="page-head"><div><h1>${title}</h1><p class="subtitle">${subtitle}</p></div><div class="actions">${route==='students'&&selectedStudent?'':button('global-payment','＋ 记缴费')}${actions}</div></header>`;}
function render(){
...
function paymentReceiptForm(text){
 openDialog('缴费已记录',`<p class="muted small">可复制下面的回执，粘贴给家长。</p>${field('收款回执',`<textarea id="payment-receipt" readonly rows="4">${esc(text)}</textarea>`)}<div class="actions"><button type="button" id="copy-receipt" class="primary">复制收款回执</button></div>`,null);
...
async function localBackupForm(){
 ...
 openDialog('本机自动备份',`<p class="muted small">选择备份后，先核对记录数量。</p>${field('备份时间','<select name="backup" disabled><option value="">正在读取…</option></select>')}<div id="local-backup-preview" aria-live="polite"></div><div id="local-backup-confirm" hidden>${notice('恢复将替换当前账本，操作前会自动备份现有记录。')}${field('确认恢复',input('confirmation','text','','placeholder="请输入：覆盖恢复" autocomplete="off"'))}</div>`,async f=>{
