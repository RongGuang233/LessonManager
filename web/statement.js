...
const labels = {payment:'缴费', refund:'退款', adjustment:'余额调整', course:'课程扣费'};
...
export function buildStatement(state, studentId, start, end) {
  ...
  const balanceReliable = student.balance_verified === true && !undatedSource.length && !uncertainAmounts && !flagged;
  const recordedOpeningCents = sum(before.filter(entry => entry.amount != null).map(entry => entry.amount));
  const recordedClosingCents = sum([recordedOpeningCents, ...within.filter(entry => entry.amount != null).map(entry => entry.amount)]);
  ...
  if (balanceReliable) notices.push('期初为开始日期之前全部账户流水的净额；期末为期初加本期流水净额，与当前余额分别计算。');
  ...
  const displayEntry = (entry, balance) => {
    ...
    return {
      ...
      label:labels[entry.type] || '账户记录', subject:course ? String(entry.record.subject ?? '') : '',
      detail:course ? `${hours(duration)} × ${money(rate)} / 小时` : '—',
      amount:entry.amount, balance, needsReview:Boolean(entry.record.needs_review),
      notes:String(entry.notes ?? ''),
    };
  };
  let running = recordedOpeningCents;
  const entries = within.map(entry => {
    if (entry.amount != null) running = sum([running, entry.amount]);
    return displayEntry(entry, balanceReliable ? running : null);
  });
  ...
  const lineFor = entry => [entry.date || '日期待核对', entry.time, entry.label, entry.subject, entry.type === 'course' ? entry.detail : '', `账户变动 ${signedMoney(entry.amount)}`, `本笔后结余 ${money(entry.balance)}`, entry.needsReview ? '记录待核对' : '', entry.notes].filter(Boolean).join(' · ');
  ...
  return {text, html, summary, entries, undated, notices};
}
