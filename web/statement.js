...
export function buildStatement(state, studentId, start, end) {
  ...
  </style></head><body><main><header><h1>${esc(student.name)} · 课时与费用对账单</h1><p class="range">本期：${esc(start)} 至 ${esc(end)}</p></header><section class="summary"><p class="balance"><span>${esc(currentLabel)}</span><strong>${esc(currentValue)}</strong></p>${metrics.slice(0,3).map(([label,value]) => `<p><span>${esc(label)}</span><strong>${esc(value)}</strong></p>`).join('')}</section>${metrics.length > 3 ? `<section class="secondary">${metrics.slice(3).map(([label,value]) => `<p><span>${esc(label)}</span><strong>${esc(value)}</strong></p>`).join('')}</section>` : ''}${periodLine ? `<p class="period">${esc(periodLine)}</p>` : ''}<h2>本期明细</h2>${parentEntries.length ? tableFor(parentEntries) : '<p class="empty">本期无已完成课程或收付款记录。</p>'}${parentUndated.length ? `<h2>日期待核对记录</h2>${tableFor(parentUndated)}` : ''}${notices.length ? `<aside class="explain">${notices.map(notice => `<p>${esc(notice)}</p>`).join('')}</aside>` : ''}<footer>课时簿 · 家长对账单</footer></main></body></html>`;
