...
- 新排课仅沿用当前学生账页或课表的学生筛选，没有明确学生时先选择；不会带入其他页面上次查看的学生。保存前显示日期、星期、起止时间和预计课费，随科目及时长更新，免费与单价待设置分别标明。排课本身不扣费。
...
- 课程页默认查看“接下来”的待上课程，按上课先后排列；所选范围没有课程时可直接查看全部待上课（含过去未确认的课程）。同一学生的流水与课程分别保留筛选范围；切换学生时课程统一恢复为“接下来＋待上课”，预设范围仅显示起止日期，自定义时展开日期输入框。流水可按缴费、扣费、退款等类型筛选，课程可按状态筛选；统计金额可直接追溯同一日期范围的记录，并返回统计。
...
- 搜索不到在读学生时，若已归档中有匹配项会提示并提供查看入口；筛选无结果时可清除筛选，不再显示空账本的添加引导。
- 从流水或课程页打开家长对账时沿用当前明确的起止日期；概览、全部记录、“接下来”或日期待定学期默认本月，可随时切换“本月”或“最近缴费起”。家长对账按日期预览，支持复制文字和打印；在浏览器打印窗口选择保存为 PDF。
...
- 统计支持“全部”、本周、本月、自定义或学期；总览和学生明细同时显示实际授课节数与小时数，每条已完成课程算 1 节，免费课也计入；空的辅助列自动隐藏，统计口径按需展开。
