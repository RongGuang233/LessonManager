...
- 在周课表空档排课，默认两小时；可每周重复、复制上周、修改时间或请假。复制前显示当前筛选范围、学生、节数与冲突；批量调课逐项预览，只同步改动过的字段，保留后续课程的单独安排。
- 点击待上课程，用 1 小时 / 2 小时快捷按钮确认实际时长，特殊价格按需展开；只确认一次扣一次费用，重复确认不重复收费。保存后可立即撤销本次记课。
- 家长随时缴费，全局入口继承当前学生或明确的筛选；无选中学生时先搜索选择。余额允许为负。请假不扣费，撤销或更正课程后余额相应调整。
...
- 原表覆盖日期与实际学期日期分开保存。学期日期待定时显示“已录入范围”，以后在设置中逐学期填写，不假定固定学期长度。
